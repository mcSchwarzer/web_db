...
data = json.load(open('src/drug_poisining_mortality_usa.json'))
vorlagedeaths = json.load(open('src/auswertungsvorlage.json'))
cnt = 0
for i in data:
    year = i["Year"]
...
with open('src/ufoauswertung.json', 'w') as outfile:
    json.dump(auswertung, outfile)

auswertung = json.load(open('src/ufoauswertung.json'))
vorlagedeaths = json.load(open('src/deathsauswertung.json'))

statelist = json.load(open('src/stateabbrev Kopie.json'))
enddatei = []
states = []
rates = []
for g in statelist:
...
allstates = json.load(open('src/stateabbrev Kopie.json'))
usaindexdeaths = [0] * 16
usaindexufos = [0] * 16
for l in allstates:
...
with open('docs/correlation_Drug_Poisining_mortality-UFO-Sightings.txt', 'w') as outfile:
    for i in enddatei:
        outfile.write(str(i)+"\n")
